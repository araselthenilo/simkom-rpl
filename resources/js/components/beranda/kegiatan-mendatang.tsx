...
const DEFAULT_KEGIATAN: Kegiatan[] = [
    {
        ...
        description: 'Eksplorasi peran Artificial Intelligence dalam ekosistem industri kreatif masa kini bersama pakar ahli.',
    ...
    },
    {
        ...
        imageAlt: 'A vibrant student music festival and cultural event at night, illuminated by warm golden and soft primary blue stage lights. Crowds of students are seen in silhouette against a brightly lit stage. The mood is celebratory and prestigious, capturing the essence of extracurricular life at STIKOM Bali in a modern, high-contrast visual style.',
        link: '#'
    },
    {
        id: 3,
        title: 'Hackathon: Code For Bali',
        description: 'Ciptakan solusi digital berbasis teknologi untuk tantangan sosial di lingkungan sekitar Bali.',
...
export default function KegiatanMendatang({ kegiatanList = DEFAULT_KEGIATAN }: KegiatanMendatangProps) {
                    ...
                    >
                        <div className="relative h-48 overflow-hidden">
                            <img
                                ...
                                src={item.image}
                            />
                            <div className="absolute top-4 left-4 rounded bg-surface/90 px-2.5 py-1.5 text-center leading-tight font-bold text-primary backdrop-blur shadow-sm">
                                <span className="block text-label-md font-semibold tracking-wider">{item.date.month}</span>
                                <span className="text-headline-sm font-extrabold">{item.date.day}</span>
                            </div>
                        </div>
                        <div className="flex flex-1 flex-col p-unit-lg">
                            <h3 className="mb-2 font-headline-sm text-headline-sm text-primary line-clamp-1">
                                {item.title}
                            </h3>
                            <p className="mb-4 line-clamp-2 font-body-sm text-on-surface-variant flex-1">
                                {item.description}
                            </p>
                            <div className="mt-auto flex items-center justify-between pt-2">
                                <div className="flex items-center gap-2 text-on-surface-variant">
                                    {getIcon(item.icon)}
                                    <span className="text-label-md">{item.location}</span>
                                </div>
                                <button className="rounded-lg bg-primary px-4 py-2 text-label-lg text-on-primary transition-all hover:bg-primary-container active:scale-95 cursor-pointer">
                                    Daftar
                                </button>
                            </div>
                        </div>
