import { Code2, Terminal, Megaphone, Music, Calendar, ChevronRight } from 'lucide-react';
...
function OrganizationIcon({ name, className }: { name: string; className?: string }) {
...
const DEFAULT_ORGANIZATIONS: Organization[] = [
    {
        ...
        icon: 'code',
        bgIcon: 'terminal',
        description: 'Fokus pada pengembangan software, kompetisi coding, dan workshop teknologi terbaru.',
        nextMeeting: {
...
export default function OrganisasiSaya({ organizations = DEFAULT_ORGANIZATIONS }: OrganisasiSayaProps) {
        ...
        <section className="py-unit-xl">
            <div className="mx-auto max-w-container-max px-margin-desktop">
                ...
                <div className="grid grid-cols-1 gap-unit-md lg:grid-cols-2">
                            ...
                            >
                                <div className="z-10">
                                    <div className="mb-4 flex items-center gap-3">
                                        <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-on-primary/10 backdrop-blur">
                                            <OrganizationIcon name={org.icon} className="h-7 w-7" />
                                        </div>
                                        <div>
                                            <h3 className="font-headline-sm text-headline-sm">{org.name}</h3>
                                    ...
                                    {org.description && (
                                        <p className="mb-6 font-body-md opacity-80">{org.description}</p>
                                    )}
                                    {org.nextMeeting && (
                                        <div className="space-y-3">
                                            <div className="flex items-center gap-3 rounded-lg border border-on-primary/10 bg-on-primary/5 p-3">
                                                ...
                                                <div>
                                                    <p className="text-label-md opacity-70">{org.nextMeeting.label}</p>
                                                    <p className="font-label-lg">{org.nextMeeting.time}</p>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                ...
                                {org.bgIcon && (
                                    <div className="absolute -right-10 -bottom-10 opacity-10 transition-transform duration-700 group-hover:scale-110">
                                        <OrganizationIcon name={org.bgIcon} className="h-[200px] w-[200px]" />
                                    </div>
                                )}
                            </div>
                        ))}
                    ...
                    <div className="flex flex-col gap-unit-md items-stretch">
                        {memberOrgs.map((org) => (
                            <div
                                key={org.id}
                                ...
                                onClick={() => org.link && window.open(org.link, '_self')}
                            >
                                <div className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-full bg-surface-container-high">
                                    <OrganizationIcon name={org.icon} className="h-7 w-7 text-primary" />
                                </div>
                                <div className="flex-grow">
                                    <h4 className="font-headline-sm text-headline-sm text-primary">{org.name}</h4>
                                    <p className="text-body-sm text-on-surface-variant">{org.statusText}</p>
                                </div>
