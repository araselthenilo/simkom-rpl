...
export default function RiwayatProfil({
    ...
    const handleAddPembina = () => {
        ...
        router.post(
            ...
            },
            {
                ...
                onError: (errors) => {
                    if (errors.nip_pembina) {
                        setValidationError(errors.nip_pembina);
                    } else {
                        setValidationError('Gagal menambahkan pembina.');
                    }
                },
                onFinish: () => {
    ...
    const handleDeletePembina = (idPembinaan: number, name: string) => {
        if (
            confirm(
    ...
    // Calculate available pembinas for the selected period
    const currentProfil = profils.find(
        (p) => p.periode_kepengurusan === selectedPeriod,
    );
    const assignedNips =
        currentProfil?.pembina?.map((p) => p.nip_pembina) || [];
    const availablePembinas = allPembina.filter(
        (p) => !assignedNips.includes(p.nip_pembina),
    );
        ...
        <main className="mx-auto flex w-full max-w-container-max flex-col gap-gutter p-margin-desktop">
                                                                    ...
                                                                    <div className="flex items-start justify-between gap-2">
                                                                        ...
                                                                        {p.id_pembinaan && (
                                                                            <Button
                                                                                ...
                                                                                variant="ghost"
                                                                                size="icon-sm"
                                                                                className="h-7 w-7 shrink-0 cursor-pointer rounded-md text-red-600 opacity-100 transition-all hover:bg-red-50 hover:text-red-700 md:opacity-0 md:group-hover/pembina:opacity-100 dark:hover:bg-red-950/30"
                                                                                title="Hapus Pembina"
                                                                            >
            ...
            <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        <DialogTitle>Tambah Pembina Organisasi</DialogTitle>
                        <DialogDescription>
                            Pilih Pembina Organisasi yang ingin ditugaskan untuk
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            {availablePembinas.length > 0 ? (
                                ...
                                <p className="text-sm text-on-surface-variant italic">
                                    Semua pembina yang terdaftar telah
                                    ditugaskan untuk periode ini.
                                </p>
                            )}
                            {validationError && (
                    ...
                    <DialogFooter>
                        ...
                        <Button
                            onClick={handleAddPembina}
                            disabled={isSubmitting || !selectedNip}
