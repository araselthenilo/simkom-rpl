...
export default function DokumentasiKegiatanIndex() {
    const { submissions = [] } = usePage<PageProps>().props;
    const [activeTab, setActiveTab] = useState<
        'Diproses' | 'Diterima' | 'Butuh Revisi' | 'Semua'
    >('Diproses');
    ...
    const filteredSubmissions = submissions.filter((item) => {
        ...
        return item.status_dokumentasi === activeTab;
    });

    const getStatusBadgeClass = (status: Submission['status_dokumentasi']) => {
        switch (status) {
            case 'Diproses':
    ...
    return (
        <>
            <Head title="Persetujuan Dokumentasi Kegiatan" />
            <main className="mx-auto w-full max-w-container-max space-y-gutter p-margin-desktop">
                {/* Header */}
                <header className="mb-unit-xl">
                    <h2 className="font-headline-lg text-headline-lg text-primary">
                        Persetujuan Dokumentasi Kegiatan
                    </h2>
                    <p className="mt-1 font-body-md text-body-md text-on-surface-variant">
                        Tinjau dan verifikasi dokumen proposal, LPJ, serta foto
                        dokumentasi dari kegiatan UKM.
                    </p>
                </header>
                ...
                <div className="overflow-hidden rounded-xl border border-outline-variant bg-surface shadow-[0px_2px_4px_rgba(26,54,93,0.05)]">
                    <div className="flex items-center justify-between border-b border-outline-variant bg-surface-container-lowest px-unit-lg py-4">
                        <h4 className="flex items-center gap-2 font-headline-sm text-headline-sm text-primary">
                            ...
                            Antrean Persetujuan Dokumentasi - {activeTab}
                        </h4>
                    ...
                    <div className="divide-y divide-outline-variant">
                        {filteredSubmissions.length === 0 ? (
                            ...
                            filteredSubmissions.map((item) => {
                                const orgName =
                                    item.kegiatan?.profil_organisasi?.organisasi
                                        ?.nama_organisasi ||
                                    item.kegiatan?.profilOrganisasi?.organisasi
                                        ?.nama_organisasi ||
                                    'Organisasi';
                                    ...
                                    >
                                        ...
                                        <div className="flex-1">
                                                ...
                                                <div>
                                                    <h5 className="flex items-center gap-2 font-label-lg text-label-lg font-semibold text-on-surface">
                                                        ...
                                                        {item.kegiatan
                                                            ...
                                                            'Kegiatan'}
                                                        <span
                                                            className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${getStatusBadgeClass(item.status_dokumentasi)}`}
                                                        >
                                                            {
                                                                item.status_dokumentasi
                                                            }
                                                        </span>
                                                    </h5>
                                            ...
                                            <div className="mt-3 flex gap-2">
                                                <Link
                                                    id={`btn-review-${item.id_dokumentasi}`}
                                                    href={
