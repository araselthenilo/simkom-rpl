...
export default function KegiatanPage({
    ...
    const filteredKegiatan = useMemo(() => {
        ...
        if (selectedType !== 'Semua') {
            result = result.filter((item) => item.jenis_kegiatan === selectedType);
        }

        if (selectedStatus !== 'Semua') {
            result = result.filter((item) => item.status_kegiatan === selectedStatus);
        }
    ...
    const handleRegisterSubmit = (e: React.FormEvent) => {
        ...
        regPost(daftar.url(selectedKegiatan.id_kegiatan), {
            onSuccess: () => {
                setIsRegOpen(false);
                toast.success(`Berhasil mendaftar ke kegiatan: ${selectedKegiatan.nama_kegiatan}`);
                regReset();
            },
            onError: (err) => {
                if (err.nim) {
                    ...
                    toast.error('Pendaftaran gagal. Silakan periksa kembali bukti transfer Anda.');
                }
            },
        });
    };
        ...
        cancelDelete(
            ...
            }),
            {
                onSuccess: () => {
                    ...
                    toast.success(`Pendaftaran ${cancelKegiatan.nama_kegiatan} berhasil dibatalkan.`);
                },
                onError: () => {
        ...
        <>
                ...
                {filteredKegiatan.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                        ...
                        <p className="mx-auto mt-2 max-w-sm font-body-md text-on-surface-variant/80">
                            Tidak ada kegiatan yang sesuai dengan kriteria filter Anda.
                        </p>
                    </div>
                ) : (
                            ...
                            return (
                                ...
                                >
                                    <div className="space-y-4">
                                        {/* Header Image/Banner */}
                                        <div className={`relative h-28 bg-gradient-to-br ${meta.gradient} ${meta.border} flex items-center justify-center border-b`}>
                                            ...
                                            <div className="absolute top-4 right-4 rounded-md bg-surface/90 px-2.5 py-1 text-center font-bold text-primary shadow-sm backdrop-blur-sm">
                                                <span className="block text-[10px] font-semibold tracking-wider">{dateBadge.month}</span>
                                                <span className="text-body-md font-extrabold">{dateBadge.day}</span>
                                            </div>

                                            <span className={`text-label-sm absolute bottom-3 left-4 rounded-full border px-2.5 py-0.5 font-semibold ${meta.border} bg-surface/80`}>
                                                {item.jenis_kegiatan}
                                            </span>
                                        </div>
                                        ...
                                        <div className="space-y-4 px-6">
                                            <div className="space-y-1">
                                                <div className="flex items-center gap-2">
                                                    <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold ${getStatusMeta(item.status_kegiatan)}`}>
                                            ...
                                            <div className="space-y-2.5 border-y border-outline-variant/30 py-3">
                                                <div className="flex items-center gap-2.5 text-on-surface-variant/90">
                                                    ...
                                                    <span className="truncate text-body-sm">{item.lokasi_kegiatan}</span>
                                                </div>
                                                <div className="flex items-center gap-2.5 text-on-surface-variant/90">
                                                    <Calendar className="h-4 w-4 shrink-0 text-primary" />
                                                    <span className="text-body-sm">{formatDate(item.tanggal_pelaksanaan)}</span>
                                                </div>
                                            ...
                                            {item.status_kegiatan !== 'Selesai' && item.status_kegiatan !== 'Dibatalkan' && (
                                                <div className="space-y-1.5">
                                                    <div className="flex justify-between text-[11px] font-semibold tracking-wide uppercase">
                                                        <span className="text-on-surface-variant/80">Kapasitas</span>
                                            ...
                                            {item.status_kegiatan === 'Dibatalkan' && item.alasan_pembatalan && (
                                                <div className="rounded-lg border border-red-500/10 bg-red-500/5 p-3 text-xs text-red-800 dark:text-red-400">
                                                    <span className="font-bold">Alasan Batal:</span> {item.alasan_pembatalan}
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-6 border-t border-outline-variant/30 p-6">
                                        {item.status_kegiatan === 'Mendatang' ? (
                                            isRegistered ? (
                                                <Button
                                                    ...
                                                    onClick={() => handleOpenCancel(item, idPeserta)}
                                                >
            ...
            <Dialog open={isRegOpen} onOpenChange={setIsRegOpen}>
                <DialogContent className="max-w-md rounded-2xl border border-outline-variant bg-surface-container-lowest p-6">
                    ...
                    {selectedKegiatan && (
                        <form className="space-y-5 py-2" onSubmit={handleRegisterSubmit}>
                            <div className="space-y-3 rounded-xl bg-surface-container-low p-4 text-body-sm">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-medium text-foreground">{selectedKegiatan.jenis_kegiatan}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-on-surface-variant">Penyelenggara:</span>
                                    <span className="font-medium text-foreground">{organisasi.nama_organisasi}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-on-surface-variant">Waktu:</span>
                                    <span className="font-medium text-foreground">{formatDate(selectedKegiatan.tanggal_pelaksanaan)}</span>
                                </div>
                                <div className="flex justify-between border-t border-outline-variant/30 pt-2 text-body-md font-semibold">
                                    <span className="text-foreground">Biaya:</span>
                                    <span>
                                        {Number(selectedKegiatan.biaya_pendaftaran) > 0 ? (
                                            <span className="text-amber-700 dark:text-amber-400">
                                                {formatRupiah(selectedKegiatan.biaya_pendaftaran)}
                                            </span>
                                        ) : (
                                ...
                                <div className="space-y-3 border-t border-outline-variant/30 pt-4">
                                    <div className="flex gap-2.5 rounded-lg border border-amber-500/20 bg-amber-500/10 p-3 text-amber-800 dark:text-amber-300">
                                        ...
                                        <div className="text-[11px] leading-relaxed">
                                            <p className="mb-1 font-bold">Petunjuk Pembayaran:</p>
                                            Silakan transfer sebesar{' '}
                                            <span className="font-bold">
                                                {formatRupiah(selectedKegiatan.biaya_pendaftaran)}
                                            </span>{' '}
                                            ke rekening Bendahara UKM penyelenggara:
                                            <p className="mt-1 font-bold">Bank BNI: 0883-2947-23 (a.n. SIMKOM RPL)</p>
                                        </div>
                                    </div>

                                    <div className="space-y-1.5">
                                        <label className="text-label-sm font-semibold text-foreground">
                                            Unggah Bukti Transfer <span className="text-error">*</span>
                                        </label>
                                        <input
                                            required
                                            ...
                                            onChange={(e) => {
                                                ...
                                                if (files && files.length > 0) {
                                                    setRegData('foto_bukti_transaksi', files[0]);
                                                }
                                            }}
                                        />
                                        <p className="text-[10px] text-on-surface-variant/70">
                                            Format: JPG, JPEG, PNG, atau PDF. Ukuran maks. 5 MB.
                                        </p>
                                        {regErrors.foto_bukti_transaksi && (
                                            <p className="text-label-sm text-error">{regErrors.foto_bukti_transaksi}</p>
                                        )}
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="flex gap-2 text-[11px] text-on-surface-variant">
                                ...
                                <span>
                                    Pendaftaran akan diajukan atas nama NIM Anda secara otomatis (
                                    <span className="font-bold text-foreground">{nim}</span>).
                                </span>
                            </div>

                            <DialogFooter className="gap-2 sm:gap-0">
                                <Button type="button" variant="outline" onClick={() => setIsRegOpen(false)}>
                                    Batal
                                </Button>
            ...
            <Dialog open={isCancelOpen} onOpenChange={setIsCancelOpen}>
                <DialogContent className="max-w-md rounded-2xl border border-outline-variant bg-surface-container-lowest p-6">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-body-sm text-on-surface-variant">
                            Apakah Anda yakin ingin membatalkan keikutsertaan Anda dalam kegiatan ini?
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    {cancelKegiatan && (
                        <form className="space-y-4" onSubmit={handleCancelSubmit}>
                            <div className="space-y-1.5 rounded-xl border border-error-container/40 bg-error-container/10 p-4 text-body-sm">
                                <div className="font-bold text-foreground">{cancelKegiatan.nama_kegiatan}</div>
                                <div className="text-[11px] text-on-surface-variant">
                                    Penyelenggara: {organisasi.nama_organisasi}
                                </div>
                                <div className="text-[11px] text-on-surface-variant">
                                    Tanggal Pelaksanaan: {formatDate(cancelKegiatan.tanggal_pelaksanaan)}
                                </div>
                            </div>
                            ...
                            <DialogFooter className="gap-2 sm:gap-0">
                                ...
                                <Button disabled={cancelProcessing} variant="destructive" type="submit">
                                    {cancelProcessing ? (
                                        <>
